...
TSLA_sentiment = pd.read_excel('TSLA_sentiment_1m.xlsx')
...
ax1.set_title('Sentiment Scores and TSLA Closing Prices by day')
ax1.plot(TSLA_merged.index, TSLA_merged['vader_mean'], label=label, color='orange')
ax1.set_ylabel('Sentiment Score')
ax1.legend(loc='upper left')
ax1.tick_params(axis='y')

label2 = 'TSLA Closing price'
ax2 = ax1.twinx()
ax2.plot(TSLA_merged.index, TSLA_merged['Close'], label=label2, color='red', linestyle='--')
ax2.set_ylabel('Closing Price')
ax2.legend(loc='upper right')
ax2.tick_params(axis='y')
...
TSLA_sentiment['senti_tag'].fillna('NaN').groupby(TSLA_sentiment['senti_tag'].fillna('NaN')).count()
